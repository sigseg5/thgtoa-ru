...
Поможет ли это руководство защитить вашу конфиденциальность от таких OSINT исследователей как Bellingcat[^13], Doxing[^14], троллей на 4chan[^15] и других, у которых нет доступа к инструментам АНБ? Скорее всего. Хотя я не так уверен насчет 4chan.

Взгляните на упрощенную модель угроз для данного руководства:

![][37]

(Обратите внимание, что "волшебные амулеты/подлодка/инсценировка смерти" и прочие шутки цитируются из источника[^10])
...
Есть также несколько более серьезных подходов к моделям угроз:
...
-   Анонимная научная деятельность в онлайне (например, доступ к научным исследованиям, когда такие ресурсы заблокированы).

-   ...

* **Обратите внимание, если у вас возникли проблемы с доступом к любой из многочисленных научных работ, упомянутых в этом руководстве, не стесняйтесь использовать Sci-Hub  (<https://en.wikipedia.org/wiki/Sci-Hub>** <sup>[[Archive.org]][45]</sup>**) для доступа к ним. Наука должна быть свободной. Любая.**
...
Если вы уже запутались, то вот видео, визуально объясняющее DNS: <https://www.youtube.com/watch?v=vrxwXXytEuI> <sup>[[Invidious]][48]</sup>
...
Так как ваш запрос не зашифрован,провайдер и/или любой другой недоброжелатель всё ещё может перехватить (используя атаку Man-in-the-middle attack[^62]) ваш запрос и будет знать, (а вероятно ещё и запишет в журнал) по каким адресам обращались с вашего IP-адреса. Провайдер может подделывать DNS-ответы, даже если вы используете частный DNS. Использование частной службы DNS бесполезно.

Ещё один приятный бонус: многие устройства и приложения будут использовать жестко закодированные DNS-серверы в обход любых системных настроек, которые вы можете задать. Это, относится к большинству (70%) смарт-телевизоров и значительной части (46%) игровых приставок[^32]. Для таких устройств вам придется заставить их[^33] перестать использовать вшитый сервис DNS, что может привести к неожиданным последствиям и они могут перестать работать правильно.
...
Небольшой примечание : **Это руководство не поддерживает и не рекомендует Cloudflare сервисы, даже если они упоминаются несколько раз в этом разделе для технического понимания.**

К сожалению, протокол TLS, используемый в большинстве HTTPS-соединений в большинстве браузеров (среди них Chrome/Brave/Ungoogled-Chromium), подвержен утечкам DNS через SNI[^37] хендшейки (это можно проверить на странице Cloudflare: <https://www.cloudflare.com/ssl/encrypted-sni/> <sup>[[Archive.org]][49]</sup> ). **На момент написания руководства, только браузеры на базе Firefox поддерживают eSNI (зашифрованный SNI**[^38] **вскоре будет переименован в ECH**[^39] **и будет поддерживаться в Firefox**[^40]**) который будет шифровать всё (в дополнение к использованию безопасного частного DNS по TLS/HTTPS) и позволит вам скрывать DNS-запросы от третьих лиц**[^41]**.** Эта опция не включена по умолчанию, так что вам придется включить её самостоятельно.

В дополнение к ограниченной поддержке браузерами, только веб-службы и CDN[^42] под управлением Cloudflare CDN поддерживают eSNI сейчас (и будут поддерживать ECH в будущем[^43]). Это означает, что eSNI (или вскоре ECH) не поддерживается (по состоянию на январь 2021 г.) многими основными платформами, такими как:
...
Некоторые страны, такие как Россия[^44] и Китай[^45], заблокируют eSNI рукопожатия на сетевом уровне, чтобы позволить шпионить и предотвратить обход цензуры. Это означает, что вы не сможете установить HTTPS-соединение с сервисом, если не позволите им увидеть, что это было.

Наконец, даже если вы используете пользовательский зашифрованный DNS-сервер (DoS или DoT) с поддержкой eSNI, этого все равно может оказаться недостаточно, так как исследования по анализу трафика[^46] показали, что всё ещё можно достаточно надёжно определять и блокировать нежелательные запросы. Только DNS через Tor смог продемонстрировать эффективную конфиденциальность DNS в недавних исследованиях, но даже это всё ещё может быть преодалено другими способами (см. [Your Anonymized Tor/VPN traffic:]).
